...
from matrx import WorldBuilder  # type: ignore
from matrx.world_builder import RandomProperty  # type: ignore
from matrx.agents import SenseCapability  # type: ignore
...
DEFAULT_WORLDSETTINGS: dict = {
    ...
    'nr_blocks_needed': 7,  # nr of drop tiles/target blocks
...
class BW4TWorld:
    ...
    def world_size(self):
        ...
        world_width = max(
            worldsettings['rooms_per_row'] * worldsettings['room_size'][0] + 2 * worldsettings['hallway_space'],
            (worldsettings['nr_drop_zones'] + 1) * worldsettings['hallway_space'] + worldsettings['nr_drop_zones']) + 2

        # calculate the total height
        world_height = nr_room_rows * worldsettings['room_size'][1] + (nr_room_rows + 1) * worldsettings[
            'hallway_space'] + worldsettings['nr_blocks_needed'] + 2
    ...
    def _addRooms(self):
        ...
        for room_nr in range(self._worldsettings['nr_rooms']):
            ...
            room_locations[room_name] = self._builder.get_room_locations(room_top_left,
                                                                         self._worldsettings['room_size'][0],
    ...
    def get_room_loc(self, room_nr):
        ...
        room_y = int(
            1 + self._worldsettings['hallway_space'] * (row + 1) + row * self._worldsettings['room_size'][1] + 1)
